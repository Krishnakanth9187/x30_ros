...
def generate_launch_description():
    ...
    robot_name = LaunchConfiguration("x30")
    ...
    gz_pkg_share = launch_ros.substitutions.FindPackageShare(package="x30_description").find(
        "x30_description"
    )

    declare_robot_name = DeclareLaunchArgument("robot_name", default_value="x30")
    ...
    gazebo_config = os.path.join(launch_ros.substitutions.FindPackageShare(
        package="x30_description"
    ).find("x30_description"), "config/gazebo.yaml")
    ...
    robot_description = {"robot_description": LaunchConfiguration("description_path")}
