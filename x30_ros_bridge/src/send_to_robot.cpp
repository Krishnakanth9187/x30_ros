...
//   const uint16_t PORT = ROBOT_PORT;
//   udp_socket_ = new UDPSocket(true);
//   udp_socket_->Connect(IP, PORT);
// }
...
void SendToRobot::start_work(){
  std::thread work_thread(std::bind(&SendToRobot::work, this));
	work_thread.detach();
}

void SendToRobot::set_send(RobotCmdSDK& cmd){
  timespec time_sdk;
  static float i = 0.00;
  // cmd.fr_leg[2].tor = 3;
  clock_gettime(0, &time_sdk);
  // cout << "send " << time_sdk.tv_sec << ". " << time_sdk.tv_nsec << endl;
...
void TimeTool::time_init(int ms){
